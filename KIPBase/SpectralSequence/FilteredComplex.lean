...
/-- 过滤层到关联分次的投影映射 `F^s A^k ⟶ gr^s A^k`。
    `gr^s A^k` 是 `F^{s+1} ↪ F^s` 的 cokernel，故此映射即 cokernel 投影 `π`。 -/
noncomputable def FilteredComplex.filToAssocGraded (FC : FilteredComplex C)
    (s k : ℤ) : Subobject.underlying.obj (FC.fil s k) ⟶ FC.assocGraded s k :=
  cokernel.π (Subobject.ofLE (FC.fil (s + 1) k) (FC.fil s k) (FC.fil_anti s k))

/-- 微分 `d` 在过滤层上的限制 `F^s A^k ⟶ F^s A^{k-1}`，
    即 `d_preserves_fil` 选取的提升（满足 `filDiff ≫ arrow = arrow ≫ d`）。 -/
noncomputable def FilteredComplex.filDiff (FC : FilteredComplex C)
    (s k : ℤ) : Subobject.underlying.obj (FC.fil s k) ⟶
...
/-- 关联分次相容性条件：路径 `F^s ⟶ gr^s ⟶d gr^s` 等于路径 `F^s ⟶d F^s ⟶ gr^s`，
    即下图交换：

    ```
    F^s A^k ----filDiff----> F^s A^{k-1}
      |                        |
    投影π                      投影π
      v                        v
    gr^s A^k -assocGradedDiff-> gr^s A^{k-1}
    ```

    这正是 `assocGradedDiff` 作为 `cokernel.desc` 的定义性质 `cokernel.π_desc`。 -/
theorem FilteredComplex.assocGradedDiff_compat (FC : FilteredComplex C)
    (s k : ℤ) :
    FC.filToAssocGraded s k ≫ FC.assocGradedDiff s k =
      FC.filDiff s k ≫ FC.filToAssocGraded s (k - 1) := by
  unfold FilteredComplex.filToAssocGraded FilteredComplex.assocGradedDiff
    FilteredComplex.filDiff FilteredComplex.assocGraded
...
/-- 第一步（纯数据）：由滤复形组装预谱序列 `PreSS C (ℤ × ℤ)`。
    只含数据字段（r₀、ssData、diffDeg、d），不含任何证明义务；
    相容性条件（d²=0、Z_succ、B_succ）在 `toSpectralSequence` 中另行证明。
    双次数 `(s, k)` = （滤次数, 同调次数），微分 `d_r` 由原复形的微分诱导。 -/
noncomputable def FilteredComplex.toPreSS (FC : FilteredComplex C)
...
/-- 第二步（证明）：调用 Basic 中的构造函数 `SpectralSequence.ofPreSS`，
    在 `toPreSS` 纯数据之上传入相容性证明，组装出谱序列。
    数据完全由 `toPreSS` 给出，本定义只提供 d_comp_d、Z_succ、B_succ 三个证明。 -/
...
谱序列双次数 `(s, k)` 处的环境对象 `V = gr^s A^k = F^s/F^{s+1}`。
称复形元素 `xl : T ⟶ F^s A^k` 是谱序列元素 `x : T ⟶ gr^s A^k`
的 **lift**，当且仅当 `xl` 经过滤层投影 `filToAssocGraded` 映到 `x`。 -/

/-- **lift 关系**：复形过滤层元素 `xl : T ⟶ F^s A^k` 是谱序列环境对象
    元素 `x : T ⟶ gr^s A^k` 的提升，当且仅当 `xl` 经投影
    `filToAssocGraded s k` 等于 `x`。 -/
def FilteredComplex.IsLift (FC : FilteredComplex C) {T : C} (s k : ℤ)
    (xl : T ⟶ Subobject.underlying.obj (FC.fil s k))
    (x : T ⟶ FC.assocGraded s k) : Prop :=
  xl ≫ FC.filToAssocGraded s k = x

/-- lift 的唯一性（模下一过滤层）：同一 `x` 的两个 lift 之差
    经过 `F^{s+1}` 分解。
    证明：两 lift 之差在 `gr^s = cokernel(F^{s+1} ↪ F^s)` 上投影为零，
    故经 `F^{s+1}` 分解（cokernel 的泛性质）。 -/
theorem FilteredComplex.isLift_sub_factors (FC : FilteredComplex C) {T : C}
...
/-- **d_r 关系的复形实现（正方向）**：
    ...
    （`xl` 经 `filDiff` 等于 `yl` 经包含 `F^{s+r} ↪ F^s` 的提升），
    则 `x` 与 `y` 在谱序列中有 `d_r` 关系。 -/
theorem FilteredComplex.differentialRelation_of_lift (FC : FilteredComplex C)
    (bnd : FC.IsBounded) (r : ℤ) (hr : 0 ≤ r) (s k : ℤ) {T : C}
    {x : T ⟶ FC.assocGraded s k}
    {y : T ⟶ FC.assocGraded (s + r) (k - 1)}
    {xl : T ⟶ Subobject.underlying.obj (FC.fil s k)}
    {yl : T ⟶ Subobject.underlying.obj (FC.fil (s + r) (k - 1))}
    (hx : FC.IsLift s k xl x) (hy : FC.IsLift (s + r) (k - 1) yl y)
...
/-- **d_r 关系的复形提升（反方向，需 T 投射）**：
    ...
    且它们之间有 `d_r` 关系，则存在复形相应过滤层中的
    lift `xl yl`，使复形层面的微分相等（模 `F^{s+r} ↪ F^s` 的包含）。
    投射性用于把页/商层面的见证沿满射投影提升回复形层面。 -/
theorem FilteredComplex.lift_of_differentialRelation (FC : FilteredComplex C)
    (bnd : FC.IsBounded) (r : ℤ) (hr : 0 ≤ r) (s k : ℤ) {T : C} [Projective T]
    {x : T ⟶ FC.assocGraded s k}
    {y : T ⟶ FC.assocGraded (s + r) (k - 1)}
...
/-- **主定理的逆否命题**：
    ...
    则 `x` 的任意 lift `xl` 的复形微分 `d(xl)` 都是 `y₁` 的 lift
    （即与 `y₁` 关联）。
    证明：若有某 lift 使 `d(xl) = yl` 不是 `y₁` 的 lift，
    则 `d_r x y`（`y` 为 `yl` 的投影）与 `d_r x y₁` 是两个关系，
    由 `differentialRelation_crossed_of_two` 得 crossing，矛盾。 -/
theorem FilteredComplex.lift_rel_of_not_crossed
    (FC : FilteredComplex C) (bnd : FC.IsBounded) (r : ℤ) (hr : 0 ≤ r) (s k : ℤ) {T : C}
    [Projective T]
    {x : T ⟶ FC.assocGraded s k}
    {y₁ : T ⟶ FC.assocGraded (s + r) (k - 1)}
    (h₁ : DifferentialRelation (FC.toSpectralSequence bnd) r ⟨s, k⟩ x y₁)
